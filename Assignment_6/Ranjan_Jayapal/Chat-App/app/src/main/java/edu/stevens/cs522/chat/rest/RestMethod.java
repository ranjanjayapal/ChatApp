...
public class RestMethod {
    ...
    public static String BASE_URL = "http://155.246.143.95:8080/chat/";
    /*
     * HTTP response
     */
    ...
    public Response perform(RegisterRequest request) {
        URL url = registerURL(request.clientID);
        if (url == null) {
    ...
    private URL registerURL(UUID clientId) {
        try {
            return new URL(BASE_URL+clientId.toString());
        }
    ...
    private URL postMessageURL(UUID clientId) {
        try {
            return new URL(BASE_URL+clientId.toString()+"/messages");
        }
        catch (MalformedURLException e) {
